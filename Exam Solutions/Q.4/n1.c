...
while(1)
{
	while(msgrcv(msgid, &message, sizeof(message), 1, IPC_NOWAIT)>0)
	{
		...
		if(news[0]>='0' && news[0]<='9')
		{
			...
			printf("Stop message sent to News Reader 2\n");
			...
			while(1)
			{
				...
				else
				{
					...
					printf("Start message sent to News Reader 2\n");
					break;
				}
			}
		}
	...
	while(msgrcv(msgid, &message, sizeof(message), 3, IPC_NOWAIT)>0)
	{
		while(1)
		{
			//received the 'stop' signal
			while(msgrcv(msgid, &message, sizeof(message), 5, IPC_NOWAIT)>0)
			{
				printf("Restarted!\n");
				goto start;
			}
